...
> GitHub Actions 在云端执行，所以即使个人电脑关机，定时任务仍会运行。定时触发使用 UTC `06:45–11:45`，对应 **北京时间 14:45–19:45 每小时一次**；第一轮在 arXiv 约 14:00 更新后留出 45 分钟缓冲，后续时段用于应对 GitHub 调度延迟。已触发的任务不再因程序层时间门控而静默跳过，避免出现 Actions 显示成功但实际没有处理。也可在 Actions 页面手动运行。
